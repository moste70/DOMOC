#include "thermo_bunk_node.hpp"
#include <cstring>

namespace domoc {

const NodeDescriptor ThermoBunkNode::kDesc = {
...
ThermoBunkNode::ThermoBunkNode()
    : NodeBase(NODE_ID_THERMO_BUNK, NodeType::THERMO_BUNK, "THERMO_BUNK") {}

const NodeDescriptor& ThermoBunkNode::descriptor() const { return kDesc; }

size_t ThermoBunkNode::build_status_payload(uint8_t* out, size_t max) {
    if (max < sizeof(ThermoStatus)) return 0;
    ThermoStatus s{};
...
void ThermoBunkNode::on_command(const CmdPayload& cmd, uint8_t /*seq*/) {
